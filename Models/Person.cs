...
public class ValidId : ValidationAttribute
{
    protected override ValidationResult IsValid(object value, ValidationContext validationContext) {
        ...
        IList<string> notValid = new List<string>();
        foreach (var item in adults){
            
            notValid.Add(item.Id.ToString());
        }
        if (notValid == null || notValid.Contains(value.ToString())) {
            return new ValidationResult("Id is already in use");
